  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      <nav
        ...
        aria-label={label}
        hidden={!open}
        className="absolute right-0 z-50 mt-2 w-56 overflow-hidden rounded-xl border border-border bg-surface-raised p-1.5 shadow-[0_18px_40px_-32px_rgb(20_20_19/0.35)]"
      >
        ...
        {items.map((item) => (
          <Link
            ...
            className="flex min-h-11 items-center rounded-md px-3 text-sm font-medium text-ink transition-colors hover:bg-surface-sunk hover:text-primary-ink"
          >
            {item.label}
          </Link>
        ))}
