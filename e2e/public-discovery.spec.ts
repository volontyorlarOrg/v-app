...
test.describe("opportunity discovery", () => {
  ...
  test("filtering by region puts the filter in the URL and narrows the list", async ({
    ...
    await page.getByLabel("Region", { exact: true }).selectOption("samarkand");
  ...
  test("a shared filtered URL reproduces the same listing", async ({ page }) => {
    ...
    await expect(page.getByLabel("Region", { exact: true })).toHaveValue("samarkand");
  ...
  test("clearing filters returns to the full list", async ({ page }) => {
    ...
    await expect.poll(async () => page.getByRole("article").count()).toBeGreaterThan(1);
  });

  test("opening an opportunity shows the facts needed to decide", async ({ page }) => {
    await page.goto("/en/opportunities");
    ...
    await expect(page.getByText("Applications close", { exact: true })).toBeVisible();
  });
  ...
  test("a signed-out visitor is asked to sign in before applying", async ({ page }) => {
  ...
  test("a removed opportunity gets its own message, not a bare 404", async ({
    ...
    await expect(
      page.getByRole("link", { name: /browse opportunities/i }),
    ).toBeVisible();
  });
});
