...
flowchart LR
  ...
  Api --> Rules["lib/record · lib/leaderboard · lib/opportunities · lib/applications · lib/profile"]
```
...
| `src/app/[locale]/(volunteer)/*/page.tsx`             | The volunteer sections and two detail pages, each `force-dynamic` and reading the backend per request; `/saved` preserves old links by redirecting to `/opportunities?view=saved`; `/settings` reads `/me` and the merge requests, owns account connections, the account's username, and sets or changes the password through a session-replacing Server Action; `/leaderboard` reads one page of `GET /leaderboard` and renders its pagination from the response's own `page`, `pageSize` and `total` |
...
| `src/lib/leaderboard/pagination.ts`                   | Page count, the clamped current page, the window of page numbers, the shown range — the only leaderboard arithmetic the frontend does; ranks and experience are the backend's                                                                                                                                                                                                                                                                                                                          |
| `src/lib/account/username.ts`                         | The username's rules, its three sources, and which of them may be renamed                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `src/lib/profile/completion.ts`                       | The nine-field completion rule and the full profile shape                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `src/lib/<domain>/actions.ts`                         | The Server Actions that write: apply (one step for an opportunity without questions), save and submit a draft, withdraw, save an opportunity, save the profile, a preference, mark notifications read, sign out                                                                                                                                                                                                                                                                                        |
...
| `src/components/brand/`                               | The logo kit's icon, wordmark and lockup, their paths, and the two provider marks                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `src/components/leaderboard/`                         | The standings table                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `src/components/account/`                             | The username section and rename form of the account page; the welcome step is its own                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `src/components/record/`                              | The participation history table                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `src/components/profile/`                             | The profile sheet, its public page link, and the profile form                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `src/components/settings/`                            | The connection list, the password connection form and the merge-request lists                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
bell, the sections, and at its foot the identity card, settings and sign out.
Every entry is a plain link — the bell is the sidebar's only popover, and the
identity card is the profile's link (`IDENTITY_ROUTE`, lit on `/profile` and
`/profile/edit`), never a menu trigger. The theme and
...
identity card, `tabBarRoutes` for the thumbs, and `ROUTE_ICONS` for the glyphs. Saved and record remain registered compatibility
...
the badge until the layout re-reads. The backend writes its notification text
in English with raw status codes, so the layout never shows it for a kind it
understands: `activityNotification()` in `src/lib/notifications/types.ts`
reads `application.submitted`, `application.reviewed` (with `data.status`) and
`attendance.resolved` (with `data.outcome`), and the layout renders the
`nav.notifications.activity` copy in the interface language, linked to the
application or to `/dashboard#history`. Joining accounts is handled the same
way through `mergeNotificationName()`; any other kind falls back to the
backend's own words. The user menu links to the consolidated
