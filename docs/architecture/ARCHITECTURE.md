...
| `src/app/[locale]/(volunteer)/*/page.tsx`             | The volunteer sections and two detail pages, each `force-dynamic` and reading the backend per request; `/saved` preserves old links by redirecting to `/opportunities?view=saved`; `/settings` reads `/me` and the merge requests, owns account connections, the leaderboard handle, and sets or changes the password through a session-replacing Server Action; `/leaderboard` reads one page of `GET /leaderboard` and renders its pagination from the response's own `page`, `pageSize` and `total` |
...
| `src/lib/account/username.ts`                         | The handle's rules, its three sources, and which of them may be renamed                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `src/components/account/`                             | The handle section and its rename form, shared by the account page and the welcome flow                                                                                                                                                                                                                                                                                                                                                                                                                |
