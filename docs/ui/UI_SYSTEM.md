...
for progress bars, `.tab-bar` for the safe-area inset, and for the welcome
flow `.onboarding-stage` (the pass's box, with the canvas fading in over the
SVG once WebGL is ready), `.onboarding-step` (the step panel's directional
entrance) and `.onboarding-rail-node` (the rail's state transitions), all
removed under reduced motion. The workspace's flat
...
| `OnboardingFlow` / `ProfileStepForm` / `PreferencesStep` / `StepRail` / `OnboardingResume`                                                         | The welcome flow: the split stage with the greeting, the pass and the rail beside the step panel; one profile step form that posts the whole profile with the other fields hidden; the notification switches saved in one action; the ordered list of steps with `aria-current="step"`, vertical beside the pass at wide widths and a strip of nodes elsewhere; the dashboard row that leads back, carrying the same strip |
...
- Reduced motion is honoured globally. Panels are always visible; the orbit
  becomes a static CSS composition and entry movement is removed. The welcome
  pass keeps its parts but loses the swing, the parallax and the tweens.
- In the welcome flow, a step change moves focus to the step's `h2`, the rail
  names each step's state in words, and both skip actions are real controls:
  a link for leaving and a button for moving on.
