...
| `Sidebar` / `SidebarNav`                                                                                                                           | The desktop shell on `shell` navy: lockup, the user card (an `AccountMenu` trigger), the MENU list of main sections, then a footer of rows: `NotificationsMenu` as a row, the theme switch as a row, sign out                                                                 |
| `MobileHeader`                                                                                                                                     | Below the large breakpoint: the lockup, the theme switch, the bell, and the avatar that opens `AccountMenu`                                                                                                                                                                   |
| `AccountMenu`                                                                                                                                      | A Radix popover holding a `nav` named "Account menu": profile, settings (and the leaderboard on a phone), the three-locale language row, sign out; the `card` variant is the sidebar user card, the `avatar` variant the phone header button                                    |
| `LeaderboardStanding` / `LeaderboardPodium` / `LeaderboardTable`                                                                                   | The viewer's card with rank and experience, the top three on a stage with the crown on first place, and the ranked table from fourth place on                                                                                                                                 |
...
| `ProfileMeter`                                                                                                                                     | The dashboard's completeness bar; the missing fields are named by `profile.completionFields`, which are short on purpose because they read as a list                                                                                                                          |
...
| `ProfileIdentity`                                                                                                                                  | The profile's own header: avatar disc, the name as `h1`, the level and completeness, a ruled band of three record figures, the bio with its empty prompt, the facts and links, and the anchor to the editor                                                                   |
...
- The sidebar appears at the large breakpoint; below it the sections live in
  the tab bar and the account routes, the leaderboard and sign out in the
  account menu behind the header avatar.
