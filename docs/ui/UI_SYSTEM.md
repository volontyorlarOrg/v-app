...
| `Sidebar` / `SidebarNav`                                                                                                                           | The desktop shell on `shell` navy: lockup, the user card (an `AccountMenu` trigger), the three sections (dashboard, opportunities, leaderboard) lit by `isSectionActive`, then a footer of rows: `NotificationsMenu` as a row, the theme switch as a row, sign out          |
...
| `AccountMenu`                                                                                                                                      | A Radix popover holding a `nav` named "Account menu": profile, settings, the three-locale language row, sign out; the `card` variant is the sidebar user card, the `avatar` variant the phone header button                                                                  |
...
- The sidebar appears at the large breakpoint; below it the sections live in
  ...
  routes and sign out in the account menu behind the header avatar.
